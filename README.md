...
CDFD 全称为 **Condition Data Flow Diagram**。本项目围绕以下主线工作：

```text
导入 CDFD -> 转换为公共图模型 -> 生成路径 -> 分析并行/选择/汇合关系 -> SVG 可视化
```
...
5. 分析路径之间的 parallel、exclusive 和 joined-output 关系。

路径定义为沿数据流边形成的源点到终点有向轨迹：

```text
IN --[x1]--> A --[x2]--> B --[x4]--> OUT_X4
```

并行路径仍然是不同的路径，其关系单独表示：
